...
class ResT(nn.Module):
    ...
    def __init__(self,
            ...
            {str(i): nn.ModuleList(
                [nn.Sequential(EMSA(input_size=input_size, stage=i, dim=self.dim * 2 ** i, head=self.head[i],
                               ...
                               MLP(dim=self.dim * 2 ** i, inter_dim=4 * self.dim * 2 ** i,
                                   dropout_ratio=self.dropout_ratio))
                 for _ in range(self.depth[i])]) for i in range(4)})
        ...
        self.classifier = nn.Linear(self.dim * 2 ** 3, self.num_classes)
    ...
    def forward(self, feats):
        ...
        feats = self.GlobalAvgPool(feats).reshape(self.batchsize, -1)
